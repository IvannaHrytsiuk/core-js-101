...
function timeSpanToString(startDate, endDate) {
  ...
  const func = (diff, num) => diff.toString().padStart(num, '0');
  return `${func(hh, 2)}:${func(mm, 2)}:${func(ss, 2)}.${func(sss, 3)}`;
}
...
function angleBetweenClockHands(date) {
  const hh = new Date(date).getUTCHours() % 12;
  const mm = new Date(date).getUTCMinutes();
  const angleHH = (hh * (360 / 12)) + (mm * (360 / (12 * 60)));
  const angleMM = mm * (360 / 60);
  const angle = Math.abs(angleHH - angleMM);
  let result = 0;
  if (angle > 180) {
